...
class HbAvssRecipient:
    ...
    def __init__ (self, publicparams, privateparams, send, recv, reconstruction=True):
        ...
        (self.t, self.n, self.crs, self.participantids, self.participantkeys, self.sid) = publicparams
        ...
        self.pc = PolyCommitNP(t=self.t, pk=self.crs)
    ...
    def receive_msg(self, sender, msg):
        ...
        if msg[1] == "send":
            ...
            (self.commit, self.encwitnesses, self.encshares, self.pk_d) = message
            self.sharedkey = self.pk_d**self.sk
        ...
        if not self.rbfinished:
            ...
            if self.check_implication(int(sender), msg[2], msg[3]):
                print ("Received Valid Implicate")
    ...
    def check_implication(self, implicatorid, key, proof):
        ...
        if not check_same_exponent_proof(proof, self.crs[0],self.pk_d, self.participantkeys[implicatorid], key):
            print ("Bad Key!")
            return False
    ...
    def send_implicate_msgs(self):
        ...
        print(self.crs[0])
        msg.append(prove_same_exponent(self.crs[0], self.pk_d,self.sk))
        for j in self.participantids:
            self.send(j, msg)
